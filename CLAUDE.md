...
- Nav tab shows a celebratory **🍻 Bars** (special-cased in `App.jsx`); Pick'em is the
  last tab. The Today tab's **"🍻 Today's bar pick"** callout (`BarOfTheDay` in
  `Today.jsx`) pulls a deterministic daily spot from the under-28th set. (The old
  per-game `WatchTeaser` was removed.)
...
- `src/components/Today.jsx` — `BarOfTheDay` callout + `FollowingPanel` (USA/Iran/Norway) →
