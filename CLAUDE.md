...
- `src/components/Today.jsx` — `FollowingPanel` (USA/Iran/Norway) →
  Now (scores/live/previews) → Latest results → News block. Each followed team shows a
  knockout-qualification pill + an expanded "what still has to happen" line, via
  `knockoutStatus()`: prefers the authoritative signal (team's real id already in a
  bracket fixture → "Into the [round]" / "Eliminated"), else format rules (top 2 of a
  group always advance; 8 best 3rds via `buildThirdPlaceRace`, showing "Nth of 12").
